...
<main id="main">
   ...
   <section id="question_to_experts" class="question_to_experts question_main mt-2">
      <div class="container">
         
         <div class="row align-items-start">
            <div class="col-lg-8 order-lg-0 order-md-1 order-1">
                <div class="row align-items-start mb-3 justify-content-start">
                    <div class="col-2">
                       <a 
                          href="{{ action('Website\StudentQuestionsAnswersController@student_questions') }}"
                          class="border-0 rounded-pill px-3 btn-sm btn btn-green">
                          <span class="">
                  ...
                  <div class="col-md-12">
                     <div class="row">
                        <div class="col-md-12 col-12 mt-0"
                           id="box_to_load"
                        >
                           <div class="row rounded bg-white px-2 py-3 position-relative shadow rounded border bg-card2">
                              <div class="col-12">
                                 <div class="row align-items-center mr-md-0 justify-content-md-start justify-content-center">
                                    ...
                                    <div class="col-sm qna_box_col text-md-left text-center my-md-0 my-2">
                                       <a                                  
                                       data-toggle="modal" data-target="#profile_modal{{$student_question->id}}"
                                       class="text-primary d-block fs-18 font-weight-bold" href="javascript:;">{{$student_question->student_name}}</a>
                                       <span class="fs-12 text-gray text-uppercase">{{ date('F d, Y', strtotime($student_question->created_at)) }}</span>
                                    </div>
                                    <form action="{{ action('Website\StudentQuestionsAnswersController@report', $student_question->id) }}" method="post" id="report_form{{$student_question->id}}">
                                       @csrf
                                    </form>
                                    <a 
                                       href="javascript:;"
                                    ...
                                    @if( session()->has('student') )
                                       @if( $student_question->user_id == session()->get('student')->id )
                                          <a data-toggle="modal" 
                                             ...
                                             href="javascript:;" class="btn btn-green btn-sm col-auto d-flex align-items-center fs-11 font-weight-bold rounded-pill bg-primary px-0 border-0 align-self-start mr-2 py-1 text-transform-none">
                                    ...
                                    @if( session()->has('student') )
                                       @if( $student_question->user_id == session()->get('student')->id )
                                          ...
                                          <a 
                                             ...
                                             @endif
                                          class="btn btn-green btn-sm col-auto d-flex align-items-center fs-11 font-weight-bold rounded-pill bg-primary px-0 border-0 align-self-start mr-2 py-1 text-transform-none"> <span class="ml-0 pl-1 pr-2"><i class="fas fa-trash ml-2"></i> Delete</span>
                                          </a>
                                       @endif
                                    @endif
                                   ...
                                   @if($student_question->total_answered!=1 && $student_question->total_answered!=0)
                                       <div class="mx-2 col-auto d-flex align-items-center fs-13 font-weight-bold rounded-pill bg-primary px-0 border border-primary align-self-start">
                                       <span class="bg-white rounded-pill px-2"> {{$student_question->total_answered}} Ans</span>
                                       </div>
                                    @endif
                                 </div>
                              </div>
                              <div class="col-md-12 col-12 mt-3">
                                    ...
                                    <div class="col-12">
                                          ...
                                          @foreach($question_tags as $question_tag)
                                             @if( !empty($question_tag) )
                                                ...
                                                @php
                                                   $has_any_tag = true;
                                                @endphp
                                             <a class="text-capitalize fs-13 border border-secondary text-secondary rounded-pill px-2 mr-1" href="javascript:;">
                                                # {{$question_tag}}
                                             </a>
                                             @endif
                                          @endforeach
                                       ...
                                       @if( session()->has('student') )
                                          @if( ($student_question->user_id == session()->get('student')->id) and !($has_any_tag) )
                                             <a class="btn btn-green btn-sm fs-10 rounded-pill py-1 border-0" 
                                             @if( session()->has('student') )
                                                data-target="#exampleModal7-update-{{$student_question->id}}"
                                             @else
                                                data-target="#exampleModal1"
                                 ...
                                 <div class="row">
                                    <div class="col">
                                       <h2 class="fs-lg-17 fs-md-14 fs-13 text-graydark text-justify">{{$student_question->name}}</h2>
                                    </div>
                                             ...
                                             <div class="modal-body">
                                                ...
                                                @csrf
                                                   ...
                                                   <div class="form-group">
                                                      ...
                                                      <span class="total_characters_typed mt-2">0</span>/4000
                                                   </div>
                                                   <div class="row align-items-center">
                                                      <div class="text-left mt-0 col-auto">
                           ...
                           @if(! empty( $student_answers->toArray() ) )
                              ...
                              @foreach($student_answers as $student_answer)
                                 <div class="
                                    ...
                                    @if($counter >= $limit)
                                       d-none
                                    @endif
                                 row px-2 py-3 position-relative shadow rounded border mt-4">
                                    <div class="col-12">
                                       <div class="row">
                                          <div class="col-12">
                                             <div class="row align-items-center justify-content-md-left justify-content-center">
                                                <div class="col-md-auto pr-md-0 text-md-left text-center">
                                                   <a 
                                                   href="#"
                                                   data-toggle="modal" data-target="#profile_modal_answer{{$student_answer->id}}"
                                                   class="d-flex align-items-center w-50px h-50px mx-auto justify-content-center border rounded-pill p-0">
                                                   
                                                   @php
                                                      $image = $student_answer->image;
                                                   ...
                                                   <img class="img-fluid rounded-pill h-40px border shadow" src="{{$image}}" alt=""></a>
                                                </div>
                                                <div class="col-sm text-md-left text-center py-md-0 py-2">
                                                   <a 
                                                   data-toggle="modal" data-target="#profile_modal_answer{{$student_answer->id}}"
                                                   class="text-primary d-block fs-14 font-weight-bold" href="javascript:;">{{$student_answer->student_name}}</a>
                                                   <span class="fs-12 text-gray text-uppercase">{{ date('F d, Y', strtotime($student_answer->created_at)) }}</span>
                                                </div>

                                                <!-- report answer -->
                                                <form action="{{ action('Website\StudentQuestionsAnswersController@report_answer', $student_answer->id) }}" method="post" id="report_answer_form{{$student_answer->id}}">
                                                   @csrf
                                                </form>
                                                <a 
                                                   href="javascript:;"
                                                   @if( session()->has('student') and !$student_answer->reported_by_me)
                                                   @php $sessiondata= session()->get('student');
                                                   @endphp
                                                      @if($sessiondata->id!=$student_answer->user_id)
                                                         onclick="return confirmation('report_answer_form{{$student_answer->id}}')"
                                                      @endif
                                                   @endif
                                                ...
                                                @php $checkuser=false; @endphp
                                                   @if( session()->has('student') )
                                                   @php $sessiondata= session()->get('student'); @endphp
                                                      @if($sessiondata->id==$student_answer->user_id)
                                                         @php $checkuser= true; @endphp
                                                      @endif
                                                    @endif
                                                ...
                                                class="col-auto d-flex align-items-center fs-13 font-weight-bold rounded-pill 
                                                   ...
                                                   @if($student_answer->reported_by_me)
                                                      border-primary 
                                                      bg-primary
                                                   @else
                                                      bg-light
                                                ...
                                                px-0 align-self-start mr-2"> <i class="fas fa-shield-check ml-2"></i> <span class="ml-0 pl-1 pr-2">
                                                ...
                                                <!-- edit answer -->
                                                @if( session()->has('student') )
                                                   @if( $student_answer->user_id == session()->get('student')->id )
                                                      <a data-toggle="modal" 
                                                         ...
                                                         href="javascript:;" class="btn btn-green btn-sm col-auto d-flex align-items-center fs-13 font-weight-bold rounded-pill bg-primary px-0 border-0 align-self-start mr-2 py-0 text-transform-none">
                                                             <span class="ml-0 pl-1 pr-2"><i class="fas fa-edit ml-2"></i>Edit</span>
                                                      </a>
                                                               ...
                                                               <div class="modal-body">
                                                                  ...
                                                                  @csrf
                                                                     ...
                                                                     <div class="form-group">
                                                                        ...
                                                                        <p class="text-danger d-none">The Answer must contains atleast 25 characters.</p>
                                                                        <span class="total_characters_typed mt-2">{{ strlen($student_answer->name) }}</span>/4000
                                                                     </div>
                                                                     <div class="row align-items-center">
                                                                        <div class="text-left mt-0 col-auto">
                                                                           <button type="submit" class="btn btn-sm px-3 btn-green border-0 rounded-pill"><span class="z-index-2">Update</span>
                                                ...
                                                @if( session()->has('student') )
                                                   @if( $student_answer->user_id == session()->get('student')->id )
                                                      ...
                                                      <a 
                                                         ...
                                                         @endif
                                                      class="btn btn-green btn-sm col-auto d-flex align-items-center fs-13 font-weight-bold rounded-pill bg-primary px-0 border-0 align-self-start mr-2 py-0 text-transform-none"> <span class="ml-0 pl-1 pr-2"><i class="fas fa-trash ml-2"></i> Delete</span>
                                                      </a>
                                                ...
                                                <!-- student profile -->
                                                <!-- Modal -->
                                                <div class="modal fade profile_modal" id="profile_modal_answer{{$student_answer->id}}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                                   <div class="modal-content shadow">
                                                      <div class="modal-header d-flex justify-content-center py-2 bg-secondary position-relative text-center">
                                                      <h5 class="modal-title fs-16" id="staticBackdropLabel">Student Profile </h5>
                                                      ...
                                                      <div class="modal-body">
                                                         <div class="student_profile">
                                                            <a data-toggle="modal" data-target="#profile_modal" href="javascript:;" class="d-flex align-items-center w-70px h-70px justify-content-center border rounded-pill p-0 mx-auto"><img class="img-fluid rounded-pill h-60px border shadow" src="{{$student_answer->student_image}}" alt=""
                                                            onerror="this.src='<?php echo asset('public/user.png'); ?>'"
                                                            ></a>
                                                            <h3 class="font-weight-bold fs-18 text-center mt-3">{{$student_answer->student_name}}</h3>
                                                            
                                                            @if( !empty($student_answer->education) )
                                                               <span class="fs-14 d-block text-center mb-2">{{$student_answer->education}} Aspirant</span>
                                                            @endif
                                                            
                                                            @if( !empty($student_answer->student_state) )
                                                            <span class="fs-14 d-block text-center mb-2"><i class="fas fa-map-marked-alt mr-2"></i>{{$student_answer->student_state}}</span>
                                                            @endif
                                                            <span class="fs-14 d-block text-center"><i class="fal fa-calendar-check mr-2"></i>Member Since {{$student_answer->student_member_since}}</span>
                                                         </div>
                                                      </div>
                                                   </div>
                                                </div>
                                          ...
                                          <div class="col-md-12">
                                             <div class="row mx-0">
                                                <div class="col-auto rounded-bottom d-inline-block text-capitalize fs-14 font-weight-bold text-dark px-0 position-relative z-index-1 bottom-n13px bg-white left-10px px-2 py-1">answer </div>
                                                <div class="col-12 pb-3 px-3 mb-3 rounded position-relative bg-white border text-justify">
                                                   <div class="p_text mt-3">
                                                   <p class="fs-md-15 fs-13 text-gray mb-0 ellipsis-5">
                                                      @php
                                                         echo substr($student_answer->name, 0)
                                                      @endphp
                                                   </p>
                                                   <p class="text fs-14 text-gray mb-2 d-none" id="read_more-{{$student_answer->id}}">
                                                      @php
                                                         echo substr($student_answer->name, 300)
                                                      @endphp
                                                   </p>

                                                   @if( strlen($student_answer->name) > 300 )
                                                      <div class="row mx-0 justify-content-end"><a id="{{$student_answer->id}}" href="javascript:;" class="bg-primary rounded-0 fs-12 py-1 px-2 text-right mb-n4 mr-n4 toggle1">Read More</a></div>
                                                   @endif
                                                   </div>
                                                </div>
                                             </div>
                           ...
                           @if( 
                              !empty($student_answers->toArray())
                              and
                              count($student_answers) >= 11
                           )
      ...
      <div class="modal-content shadow">
         ...
         <div class="modal-body">
            <form action="{{ action('Website\StudentQuestionsAnswersController@ask_question') }}" method="post">
               ...
               <div class="row align-items-center">
                  <div class="form-group col mb-0">
                     <input type="text" multiple class="tagsInput" value="" data-initial-value='' data-user-option-allowed="true" placeholder="Select Tags" data-url="{{ action('Website\StudentQuestionsAnswersController@tags_for_questions') }}" data-load-once="true" name="tags" />
                  </div>
      ...
      $(document).on('submit', 'form', function(event) {
         ...
         if(minlength != '' || maxlength != '') {
            ...
            if(textarea_value.length >= minlength && textarea_value.length <= maxlength) {
               ...
               console.log($("textarea",this).parent().find('p.text-danger'));
