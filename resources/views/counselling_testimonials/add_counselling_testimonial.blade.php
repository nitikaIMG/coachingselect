...
<div class="card">
    <div class="card-header">Add Counselling Testimonial</div>
    <div class="card-body">
        <div class="sbp-preview">
            <div class="sbp-preview-content">
                <form action="{{ action('CounsellingTestimonialsController@add_counselling_testimonial') }}" method="post" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="stars" placeholder="Enter Stars" required maxlength="3" onchange="return is_correct_stars()" onkeypress="return isNumberKey(event)">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="form-group">
    ...
    <script>
        $(document).on('change', '#country_id', function() {
            $.ajax({
                type: 'POST',
                url: '{{action("CounsellingTestimonialsController@states")}}',
                data: {
                    country_id: $(this).val(),
                    _token: '{{csrf_token()}}'
                },
    ...
    <script>
        $(document).on('change', '#state_id', function() {
            $.ajax({
                type: 'POST',
                url: '{{action("CounsellingTestimonialsController@cities")}}',
                data: {
                    state_id: $(this).val(),
                    _token: '{{csrf_token()}}'
                },
