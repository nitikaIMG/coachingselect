...
class TrendingTodayDirectController extends Controller
{

    public function add_trending_today_direct()
    {

        if (request()->isMethod('get')) {

            return view('trending_today_direct.add_trending_today_direct');
        } else {

            $input = request()->except('_token');

            $is_exists = DB::table('trending_today_direct')
                ->where('title', $input['title'])
                ->exists();

            if ($is_exists) {
                return back()->with('error', 'This Trending today Direct already exists');
            }
            ...
            DB::table('trending_today_direct')->insert($input);

            return redirect()
                    ->action('TrendingTodayDirectController@view_trending_today_direct')
                    ->with('success', 'Trending today Direct Added successfully');
        }
    }

    public function edit_trending_today_direct()
    {
        ...
        if (request()->isMethod('get')) {
            ...
            DB::table('trending_today_direct')->where('id', $input['id'])->update($input);

            return redirect()->back()->with('success', 'Trending today Direct Updated successfully');
        }
    }

    public function delete_trending_today_direct()
    {

        $input = request()->except('_token');

        $old_status = DB::table('trending_today_direct')->where('id', $input['id'])->value('status');

        $new_status = ($old_status == 'enable') ? 'disable' : 'enable';

        DB::table('trending_today_direct')->where('id', $input['id'])->update(['status' => $new_status]);

        if($new_status == 'enable')
            return redirect()->back()->with('success', 'Trending today Direct ' . $new_status . ' successfully');
        else 
            return redirect()->back()->with('danger', 'Trending today Direct ' . $new_status . ' successfully');
    }

    public function view_trending_today_direct()
    {

        return view('trending_today_direct.view_trending_today_direct');
    }

    public function view_trending_today_direct_dt(Request $request)
    {

        $columns = array(
            0 => 'trending_today_direct.id',
            1 => 'trending_today_direct.title',
            2 => 'trending_today_direct.url',
            3 => 'trending_today_direct.created_at',
        );
        ...
        $query = DB::table('trending_today_direct');

        if (request()->has('name')) {
            $name = request('name');
            if ($name != "") {
                $query->where('trending_today_direct.title', 'LIKE', '%' . $name . '%');
            }
        }
        if(request()->has('start_date')){
			...
			if($start_date!=""){
				$query = $query->whereDate('trending_today_direct.created_at', '>=',date('Y-m-d',strtotime($start_date)));
			}
		}
		...
		if(request()->has('end_date')){
			...
			if($end_date!=""){
				$query = $query->whereDate('trending_today_direct.created_at', '<=',date('Y-m-d',strtotime($end_date)));
			}
		}
        ...
        if (!empty($posts)) {
            ...
            foreach ($posts as $post) {
                ...
                $new_status = ($post->status == 'enable') ? '<a class="btn btn-sm btn-danger" href="' . action('TrendingTodayDirectController@delete_trending_today_direct', 'id=' . $post->id) . '" onclick="' . $confirm . '">Disable</a></div>' : '<a class="btn btn-sm btn-outline-danger" href="' . action('TrendingTodayDirectController@delete_trending_today_direct', 'id=' . $post->id) . '" onclick="' . $confirm . '">Enable</a></div>';
				...
				<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl" role="document">
					...
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalLongTitle">Edit Trending today Direct</h5>
					...
					<div class="modal-body">
						<div class="container">
                            <form action="' . action('TrendingTodayDirectController@edit_trending_today_direct') . '" class="form" enctype="multipart/form-data" method="post">                                
                                ' . csrf_field() . '
                                ...
                                <div class="form-group">
                                <input type="text" class="form-control" value="' . $post->title . '" name="title" required placeholder="Enter Trending today Direct Title">
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
