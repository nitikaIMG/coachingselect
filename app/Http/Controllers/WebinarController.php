...
class WebinarController extends Controller
{
    ...
    public function add_webinar()
    {
        ...
        if (request()->isMethod('get')) {
            ...
            return view('webinar.add_webinar');
        } else {

            $input = request()->except('_token');

            $is_exists = DB::table('webinar')
                ->where('title', $input['title'])
                ->exists();

            if ($is_exists) {
                return back()->with('error', 'This webinar already exists');
            }
            ...
            DB::table('webinar')->insert($input);

            return redirect()
                    ->action('WebinarController@view_webinar')
                    ->with('success', 'Webinar Added successfully');
        }
    }
    ...
    public function edit_webinar()
    {
        ...
        if (request()->isMethod('get')) {
            ...
            DB::table('webinar')->where('id', $input['id'])->update($input);

            return redirect()->back()->with('success', 'Webinar Updated successfully');
        }
    }
    ...
    public function delete_webinar()
    {
        ...
        $old_status = DB::table('webinar')->where('id', $input['id'])->value('status');

        $new_status = ($old_status == 'enable') ? 'disable' : 'enable';

        DB::table('webinar')->where('id', $input['id'])->update(['status' => $new_status]);

        if($new_status == 'enable')
            return redirect()->back()->with('success', 'Webinar ' . $new_status . ' successfully');
        else 
            return redirect()->back()->with('danger', 'Webinar ' . $new_status . ' successfully');
    }

    public function view_webinar()
    {

        return view('webinar.view_webinar');
    }

    public function view_webinar_dt(Request $request)
    {
        ...
        $query = DB::table('webinar');

        if (request()->has('name')) {
            $name = request('name');
            if ($name != "") {
                $query->where('webinar.title', 'LIKE', '%' . $name . '%');
            }
        }
        if(request()->has('start_date')){
			...
			if($start_date!=""){
				$query = $query->whereDate('webinar.created_at', '>=',date('Y-m-d',strtotime($start_date)));
			}
		}
		...
		if(request()->has('end_date')){
			...
			if($end_date!=""){
				$query = $query->whereDate('webinar.created_at', '<=',date('Y-m-d',strtotime($end_date)));
			}
		}
        ...
        if (!empty($posts)) {
            ...
            foreach ($posts as $post) {
                ...
                $new_status = ($post->status == 'enable') ? '<a class="btn btn-sm btn-danger" href="' . action('WebinarController@delete_webinar', 'id=' . $post->id) . '" onclick="' . $confirm . '">Disable</a></div>' : '<a class="btn btn-sm btn-outline-danger" href="' . action('WebinarController@delete_webinar', 'id=' . $post->id) . '" onclick="' . $confirm . '">Enable</a></div>';
				...
				<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl" role="document">
					...
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalLongTitle">Edit webinar</h5>
					...
					<div class="modal-body">
						<div class="container">
                            <form action="' . action('WebinarController@edit_webinar') . '" class="form" enctype="multipart/form-data" method="post">                                
                                ' . csrf_field() . '
                                ...
                                <div class="form-group">
                                <input type="text" class="form-control" value="' . $post->title . '" name="title" required placeholder="Enter webinar Title">
                                </div>
                                ...
                                <div class="form-group">
                                    <input type="text" class="form-control" value="' . $post->author . '" name="author" placeholder="Enter Webinar Author" required>
                                </div>

                                <div class="form-group">
                                    <input type="text" class="form-control datetimepickerget" value="' . $post->date . '" name="date" placeholder="Enter Date">
                                </div>
                            
                                <div class="form-group">
                                    <input type="time" class="form-control" value="' . $post->time . '" name="time" placeholder="Enter Time">
                                </div>
